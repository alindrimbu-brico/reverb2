...
const quotes: QuoteType[] = [
  {
    text: "Trauma nu este ceea ce ți se întâmplă ție. Trauma este ceea ce se întâmplă în interiorul tău ca rezultat a ceea ce ți s-a întâmplat.",
    author: "Dr. Gabor Maté"
  },
  {
    text: "Corpul tău ține un scor pe care mintea încearcă să-l ignore. Perturbarea cronică a homeostaziei duce la boală fizică și spirituală.",
    author: "Dr. Gabor Maté"
  },
  {
    text: "Dacă nu poți spune NU, corpul tău o va face pentru tine sub formă de simptom biologic.",
    author: "Dr. Gabor Maté"
  },
  {
    text: "Trauma determină o redirecționare fundamentală a sistemului nervos pentru supraviețuire, sacrificând pacea homeostaziei.",
    author: "Dr. Bessel van der Kolk"
  },
  {
    text: "Homeostazia este starea de armonie biologică. Suprastimularea înseamnă că creierul tranzacționează sănătatea viitoare pentru supraviețuirea din prezent.",
    author: "Dr. Andrew Huberman"
  },
  {
    text: "Liniștea nu înseamnă absența mișcării, ci acordarea perfectă a mișcării biologice cu vibrația Universului.",
    author: "Perspectivă Spirituală Guru"
  }
];
...
export default function HomeostasisGraph() {
  ...
  const getPhysiologicalState = () => {
    if (activeBalance < 0.2) return { text: "Parasimpatic Profund (Repaus Meditativ)", color: "text-blue-400", wave: "Unde Delta / Theta (0.5 - 7 Hz)" };
    if (activeBalance < 0.48) return { text: "Homeostazie Dinamică (Sănătate Celulară)", color: "text-cyan-400", wave: "Unde Alfa (8 - 12 Hz) - Meditație" };
    if (activeBalance >= 0.48 && activeBalance <= 0.52) return { text: "Punctul de Echilibru (Axa Zero)", color: "text-white", wave: "Unde Mu / SMR (12 - 15 Hz) - Focus calm" };
    if (activeBalance > 0.52 && activeBalance < 0.8) return { text: "Activare Simpatică (Alertă / Focus)", color: "text-orange-400", wave: "Unde Beta (15 - 30 Hz) - Excitare" };
    return { text: "Suprastimulare și Toxicitate (Cortisol/Adrenalină)", color: "text-red-500 animate-pulse", wave: "Unde Gamma / High Beta (30 - 100 Hz)" };
  };

  const stateDesc = getPhysiologicalState();

  // Locked Diagnostic Detailed Content Data (Highly explicit clinical/spiritual level)
  const getDiagnosticDetails = () => {
    if (activeBalance < 0.2) {
      return {
        stateName: "Stare Zen / Dominanță Parasimpatică (Vagotonie)",
        explanation: "Reprezintă activarea profundă a ramurii ventrale mielinizate a nervului vag (Calea Ventro-Vagală), conducând la eliberarea masivă de acetilcolină la nivelul nodului sinoatrial pentru încetinirea controlată a ritmului cardiac (bradicardie fiziologică de repaus). Axa HPA este complet amortizată, permițând refacerea depozitelor de glicogen hepatic, resintetizarea ATP-ului intracelular și normalizarea baseline-ului de tirozin-hidroxilază. Creierul intră în unde Delta și Theta profunde, stări bio-electrice ce facilitează drenajul limfatic cerebral (sistemul glimfatic) și consolidarea memoriei imunologice.",
        ...
        advice: "• Respirație de tip 'Physiological Sigh': Efectuați două inspirații rapide pe nas (a doua fiind scurtă și forțată pentru re-expandarea alveolelor pulmonare), urmate de un expirație lungă, lentă și pasivă pe gură. Repetați de 5 ori pentru a stimula instantaneu receptorii de presiune baro-vagală.\n• Meditație Transcendentă și Samadhi: Cultivați tăcerea introspectivă absolută, orientând atenția spre pauza naturală din apneea post-expirație. Aceasta ajută la dizolvarea nodurilor de tensiune somatică acumulate istoric.\n• Atenție Clinică (Avertisment): Evitați alunecarea din starea activă ventro-vagală în cea de dominanță dorso-vagală (starea de îngheț/freeze, disociere emoțională, letargie cronică sau depresie vegetativă). Reactivați-vă ușor prin mișcări somatice conștiente sau expunere scurtă la frig."
      };
    } else if (activeBalance < 0.48) {
      return {
        stateName: "Homeostazie Dinamică (Axa de Liniște Alfa)",
        explanation: "Acesta este standardul de aur al echilibrului allostatic de zi cu zi. Axa hipotalamo-pituitaro-suprarenală (HPA) funcționează într-o buclă perfectă de feedback negativ, menținând secreția de cortisol și ACTH în limite ideale de sănătate. Fuzionarea tonusului simpatic cu cel parasimpatic permite celulelor imunitare (limfocitele T și celulele NK) să funcționeze optim, suprimând citokinele pro-inflamatorii (IL-6 și TNF-alpha). Dopamina este eliberată în mod tonic (stabil și constant) din aria tegmentală ventrală (VTA) către nucleul accumbens, prevenind fenomenul de down-regulation al receptorilor D2 și menținând motivația curată, lipsită de pulsiuni adictive (craving).",
        ...
        advice: "• Teoria Afectivă Gabor Maté: Cultivați capacitatea de a recunoaște și exprima emoțiile reprimate (mânia sănătoasă). Învățați să spuneți un 'NU' autentic celor din jur pentru ca corpul dumneavoastră să nu fie nevoit să spună 'NU' sub formă de boală autoimună sau oboseală cronică.\n• Sincronizare Circadiană: Expuneți-vă ochii la lumina solară directă în primele 30 de minute de la trezire pentru a genera pulsul natural de cortisol matinal și a bloca secreția tardivă de melatonină.\n• Grounding și Co-Regulare: Practicați contactul fizic direct cu pământul (earthing) și consolidați legăturile cu o comunitate sigură emoțional (Sangha), esențială pentru co-regularea automată a sistemului nervos prin neuroni-oglindă."
      };
      ...
      return {
        ...
        advice: "• Respirație Coerentă (Resonance Frequency): Respirați într-un ritm de 5 secunde pe inspirație și 5 secunde pe expirație (aproximativ 6 respirații pe minut). Acest ritm activează rezonatorul pulmonar natural și stabilizează tensiunea arterială.\n• Wu Wei (Acțiune fără Efort): Adoptați starea mentală taoistă a non-atașamentului. Acționați din flux, ghidat de prezență pură, nu de frici sau ambiții egoice anxioase. Observați fluctuațiile impulsurilor biologice fără a vă identifica cu ele.\n• Echilibru Somatic: Faceți pauze scurte de conștientizare corporală pe parcursul zilei pentru a vă realinia centrul de greutate și a calma activitatea cortexului cingulat anterior."
      };
    } else if (activeBalance > 0.52 && activeBalance < 0.8) {
      return {
        stateName: "Activare Simpatică (Alertă Focalizată / Beta Drive)",
        explanation: "Sistemul nervos simpatic mobilizează resursele energetice prin activarea axei simpato-adreno-medulare (SAM). Locus coeruleus își mărește rata de descărcare, inundând cortexul cu noradrenalină pentru a asigura o viziune focalizată (atenție selectivă pe obiectiv). Glicogenul din ficat este transformat în glucoză circulantă pentru a hrăni mușchii și creierul. Frecvența cardiacă urcă prin stimularea receptorilor beta-1 adrenergici. Dopamina trece de la eliberarea tonică la cea fazică (vârfuri de excitare), generând dorință intensă, productivitate ridicată și focalizare pe recompensă exterioară.",
        ...
        advice: "• Delimitare Allostatică: Limitați perioadele de efort simpatic la reprize de maximum 90 de minute (cicluri ultradiene). Urmați fiecare repriză de cel puțin 10 minute de de-focalizare vizuală (privit în depărtare, plimbare fără ecrane).\n• Grounding Somatic rapid (Tehnica 5-4-3-2-1): Identificați în spațiu 5 lucruri pe care le vedeți, 4 pe care le puteți atinge, 3 pe care le auziți, 2 pe care le mirosiți și 1 pe care îl gustați pentru a ancora creierul în realitatea fizică, oprind buclele de ruminație.\n• Transmutare Conștientă (Karma Yoga): Canalizați impulsul adrenergic către creație și muncă focalizată, fără a lăsa presiunea să degenereze în iritabilitate sau anxietate de performanță."
      };
    } else {
      return {
        stateName: "Exhaustie HPA & Alertă Toxică (Suprastimulare Cronică)",
        explanation: "Starea de degradare allostatică severă. Receptorii de glucocorticoizi din hipocamp și cortexul prefrontal sunt down-regulați din cauza expunerii prelungite la cortisol (rezistență la cortisol), perturbând bucla de oprire a stresului. Amigdala devine hiper-activă și hipertrofiată, interpretând orice stimul neutru ca pe o amenințare vitală. Glutamatul se acumulează în exces în sinapse, declanșând neurotoxicitate și atrofie dendritică în zonele cognitive. Receptorii de dopamină D2 sunt drastic reduși ca număr, instalând anhedonia de bază (imposibilitatea de a simți plăcere naturală), ceea ce împinge organismul spre consum compulsiv sau epuizare imunitară totală.",
        ...
        advice: "• Resetare Senzorială Urgentă: Blocați complet stimulii vizuali și auditivi digitali (modul avion, deprivare senzorială în cameră întunecată). Suplimentați sub îndrumare clinică cu adaptogene puternice (Ashwagandha KSM-66, Rhodiola Rosea, Magneziu L-Threonat pentru traversarea barierei hemato-encefalice).\n• Somatic Experiencing (Terapia Dr. Peter Levine): Permiteți corpului să tremure în mod natural pentru a descărca energia de supraviețuire blocată în mușchi. Nu blocați plânsul sau tremurul; ele sunt mecanismele ancestrale de descărcare a traumei somatizate.\n• Rendare Spirituală Radicală (Surrender): Practicați abandonul total al nevoii de control (Vairagya). Acceptați vulnerabilitatea și limitele corpului fizic ca fiind templul sacru al conștiinței. Reconectați-vă cu pământul prin contact direct și respirație abdominală profundă."
      };
    }
  };
    ...
    >
      ...
      <div className="max-w-5xl mx-auto w-full">
        ...
        >
          <div className="flex flex-wrap justify-center gap-4 items-center">
            <div className="inline-flex items-center space-x-2 bg-neutral-900 border border-neutral-800 px-4 py-2 rounded-full text-xs font-mono tracking-widest text-neutral-400 uppercase shadow-lg">
              ...
              <span>Ghidul Interactiv de Echilibru Simpatic/Parasimpatic</span>
            </div>
          </div>
          ...
          <p className="text-lg text-neutral-400 max-w-2xl mx-auto font-light leading-relaxed">
            Explorați axa neurală mișcând mouse-ul. **Faceți click** pe simulator pentru a bloca și investiga în detaliu o stare specifică.
          </p>
        </motion.div>
          ...
          <div className="lg:col-span-7 flex flex-col justify-between p-8 rounded-3xl bg-neutral-900/40 border border-neutral-800/80 backdrop-blur-xl relative overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center justify-between gap-4 mb-6 z-10 relative">
              ...
              <div className="text-right text-neutral-400 font-mono text-[10px]">
                <div className="uppercase tracking-widest text-[8px] text-neutral-500">Coerență Sinaptică</div>
            ...
            <div className="grid grid-cols-12 gap-6 mb-6 items-stretch">
              ...
              <div className="col-span-9 relative rounded-2xl overflow-hidden bg-neutral-950/80 border border-neutral-800/60 shadow-inner group cursor-crosshair min-h-[220px]">
                ...
                <div className="absolute bottom-4 left-4 z-20 text-[7px] font-mono tracking-widest text-neutral-500 uppercase flex items-center gap-1.5 bg-neutral-950/80 px-2 py-0.5 rounded-full border border-neutral-800/40 pointer-events-none">
                  ...
                  <span>{isLocked ? "CLICK PENTRU DEBLOCARE" : "CLICK PENTRU DIAGNOSTICARE"}</span>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-4 z-10 relative">
              ...
              >
                ...
                <span>Extrage Adevărul Neurologic</span>
              </button>
            </div>
              ...
              <div className="p-8 rounded-3xl bg-neutral-900/55 border border-amber-500/30 backdrop-blur-2xl relative shadow-2xl space-y-8">
                ...
                <div className="flex items-center justify-between border-b border-neutral-850 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[9px] font-mono text-neutral-500 uppercase tracking-widest block">RAPORT DE DIAGNOSTICARE NEURALĂ</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-sm font-bold uppercase tracking-widest text-[#3b82f6] flex items-center gap-2">
                      ...
                      <span>Analiză Neuro-Fiziologică</span>
                    </h4>
                    <p className="text-sm text-neutral-300 leading-relaxed font-light">
                      {diag.explanation}
                    </p>
                    <div className="p-4 rounded-2xl bg-neutral-950/60 border border-neutral-850 space-y-2">
                      <div className="text-[10px] font-mono text-neutral-500 uppercase tracking-wider">Indicatori Cheie:</div>
                      <div className="grid grid-cols-2 gap-2 text-xs font-mono text-neutral-300">
                        <div>Proces A (Dopamină): <span className="text-blue-400 font-bold">{Math.floor(heightA)}%</span></div>
                        <div>Proces B (Stres): <span className="text-red-400 font-bold">{Math.floor(heightB)}%</span></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h4 className="text-sm font-bold uppercase tracking-widest text-amber-500 flex items-center gap-2">
                      ...
                      <span>Ghidul de Vindecare (Recomandare)</span>
                    </h4>
        ...
        <div className="mt-16 text-center">
          <Link 
            href="/showcase/neuro-recovery/homeostasis"
          ...
          >
            ...
            <span>Documentație Completă Homeostazie</span>
          </Link>
        </div>
